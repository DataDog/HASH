...
const generate = async (appFolder, options) => {
    ...
    if (!availableTemplates.includes(template)) {
        console.error(
            'Template %s not exists, the available templates are (%s)',
            template,
    ...
    if (fs.existsSync(appFolder)) {
        console.error(
            'Target folder: %s is already exists, please remove it or change the target folder',
            appFolder
        );
        process.exit(1);
    }
...
const replaceTemplateContents = (appFolder, options) => {
    let initContents = fs.readFileSync(appFolder + '/init.yaml', {
        encoding: 'utf8',
    });

    initContents = initContents.replace(
        '{%HoneypotName%}',
...
const generateBasedOnSwagger = async (appFolder, options) => {
    ...
    try {
        ...
        for (const path in api.paths) {
            ...
            fs.writeFileSync(
                appFolder + '/templates/' + pathJSON.id + '.yaml',
                pathYAML
            );
        }
    } catch (err) {
...
const generatePath = (appFolder, path, requests) => {
    ...
    for (const method in requests) {
        ...
        let res = buildResponse(
            request.responses[Object.keys(request.responses)[0]].schema,
            pathToRoute
        ); //Currently support the first response
        fs.writeFileSync(
            appFolder + '/templates/resources/' + DatafileName,
    ...
    if (schema.type == 'object') {
        ...
        for (const propertyKey in schema.properties) {
            ...
            if (propertyVal.type == 'object') {
                ...
                tmpRes[propertyKey] = autoDiscoverRandomization(
                    propertyKey,
                    propertyVal.type,
                    pathToRoute
...
const autoDiscoverRandomization = (keyName, type, pathToRoute) => {
    ...
    if (
        ['address', 'street1', 'street2', 'address1', 'address2'].includes(
            keyName
        )
    ) {
        //its a name
